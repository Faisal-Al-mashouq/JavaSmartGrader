...
def construct_prompt(
    *,
    ...
    return (
        ...
        "- Include one rubric_breakdown entry per rubric criterion.\n"
        "- Use criterion_id_or_name as a single string, not an array.\n"
        "- feedback.summary is required.\n"
        "- Each rubric_breakdown item must include evidence_from_code_or_logs.\n"
        "- confidence is a top-level field between 0 and 1 (not feedback_score).\n"
        "- Put error classification notes in error_classification.notes "
        "(not error_classification_notes).\n"
        "- error_classification must use handwriting_ocr_suspected "
        "(not handwritten_ocr_suspected).\n"
