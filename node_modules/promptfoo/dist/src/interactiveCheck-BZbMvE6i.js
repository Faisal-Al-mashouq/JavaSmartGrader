...
function canUseInteractiveUI() {
	if (!process.stdout.isTTY) {
		logger_default.debug("Interactive UI not available: stdout is not a TTY");
		return false;
	}
	return true;
}
...
function shouldUseInkUI() {
	if (!isInteractiveUIEnabled()) {
		logger_default.debug("Ink UI disabled: not explicitly enabled (set PROMPTFOO_ENABLE_INTERACTIVE_UI=true)");
		return false;
	}
	if (!canUseInteractiveUI()) {
		logger_default.debug("Ink UI disabled: TTY required but not available");
		return false;
	}
	logger_default.debug("Ink UI enabled via PROMPTFOO_ENABLE_INTERACTIVE_UI");
	return true;
}

//#endregion
export { shouldUseInkUI as n, canUseInteractiveUI as t };
//# sourceMappingURL=interactiveCheck-BZbMvE6i.js.map