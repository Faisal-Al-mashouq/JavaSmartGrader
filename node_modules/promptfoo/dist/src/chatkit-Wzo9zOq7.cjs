...
var ChatKitBrowserPool = class ChatKitBrowserPool {
	...
	*/
	static getInstance(config) {
		if (!ChatKitBrowserPool.instance) {
			...
			if (config.maxConcurrency !== void 0 && config.maxConcurrency !== existing.maxConcurrency || config.headless !== void 0 && config.headless !== existing.headless) require_logger.logger_default.warn("[ChatKitPool] Pool already exists with different config, ignoring new config", {
				existing: {
	...
	static resetInstance() {
		if (ChatKitBrowserPool.instance) {
			ChatKitBrowserPool.instance.shutdown().catch((err) => {
				require_logger.logger_default.debug("[ChatKitPool] Error during shutdown:", { error: String(err) });
			});
			ChatKitBrowserPool.instance = null;
		}
	}
	...
	*/
	setTemplate(templateKey, html) {
		if (this.templates.get(templateKey) !== html) {
			...
			require_logger.logger_default.debug("[ChatKitPool] Registered template", { templateKey });
	...
	*/
	...
	async doInitialize() {
		require_logger.logger_default.debug("[ChatKitPool] Initializing browser pool", { maxConcurrency: this.config.maxConcurrency });
		...
		await new Promise((resolve, reject) => {
			...
			this.server.listen(this.config.serverPort, () => {
				...
				require_logger.logger_default.debug("[ChatKitPool] Server started", { port: this.serverPort });
				resolve();
			});
		});
		try {
		...
		require_logger.logger_default.debug("[ChatKitPool] Browser pool initialized");
	}
	/**
	...
	*/
	async acquirePage(templateKey) {
		...
		if (available) {
			...
			require_logger.logger_default.debug("[ChatKitPool] Acquired existing page", {
				templateKey,
		...
		if (needsRefresh) {
			...
			require_logger.logger_default.debug("[ChatKitPool] Acquired and refreshed page", {
				templateKey,
				poolSize: this.pages.length
			});
			return needsRefresh;
		...
		if (this.pages.length < this.config.maxConcurrency) {
			...
			require_logger.logger_default.debug("[ChatKitPool] Created new page", {
				templateKey,
				poolSize: this.pages.length
			});
			return pooledPage;
		}
		require_logger.logger_default.debug("[ChatKitPool] Waiting for available page", {
			templateKey,
			poolSize: this.pages.length,
	...
	*/
	async releasePage(pooledPage) {
		...
		try {
			...
			require_logger.logger_default.warn("[ChatKitPool] Failed to reset page, recreating", { error });
			...
			try {
				...
				require_logger.logger_default.warn("[ChatKitPool] Failed to create replacement page", { error: createError });
	...
	async tryServeWaiters() {
		while (this.waitQueue.length > 0 && this.pages.length < this.config.maxConcurrency) {
			...
			try {
				...
				require_logger.logger_default.debug("[ChatKitPool] Created page for waiting request", {
					templateKey: waiter.templateKey,
					poolSize: this.pages.length,
					remainingWaiters: this.waitQueue.length
				});
			} catch (error) {
				require_logger.logger_default.warn("[ChatKitPool] Failed to create page for waiter", {
					templateKey: waiter.templateKey,
					error
				});
	...
	*/
	scheduleIdleShutdown() {
		...
		if (this.pages.filter((p) => p.inUse).length === 0 && this.waitQueue.length === 0 && this.pages.length > 0) {
			require_logger.logger_default.debug("[ChatKitPool] Pool idle, scheduling shutdown", { delay: IDLE_SHUTDOWN_DELAY_MS });
			this.idleTimer = setTimeout(() => {
				if (this.pages.filter((p) => p.inUse).length === 0 && this.waitQueue.length === 0) {
					require_logger.logger_default.debug("[ChatKitPool] Auto-shutting down idle pool");
					this.shutdown().catch((err) => {
						require_logger.logger_default.debug("[ChatKitPool] Error during idle shutdown", { error: String(err) });
					});
					ChatKitBrowserPool.instance = null;
				}
	...
	*/
	...
	async refreshPooledPage(pooledPage) {
		require_logger.logger_default.debug("[ChatKitPool] Refreshing page", { timeout: PAGE_REFRESH_TIMEOUT_MS });
	...
	*/
	async shutdown() {
		require_logger.logger_default.debug("[ChatKitPool] Shutting down");
		this.cancelIdleTimer();
		if (this.waitQueue.length > 0) {
			require_logger.logger_default.debug("[ChatKitPool] Clearing pending waiters", { count: this.waitQueue.length });
			this.waitQueue = [];
		}
		...
		require_logger.logger_default.debug("[ChatKitPool] Shutdown complete");
	}
};
			...
			if (isOpenAICdnUrl(url)) try {
				...
				if (result.text && result.text.length > 0) {
					...
					if (trimmed === "ApproveReject" || trimmed === "Approve" || trimmed === "Reject") {
						require_logger.logger_default.debug("[ChatKitProvider] Skipping approval button text", { text: trimmed });
						continue;
					}
					const cleaned = cleanAssistantResponse(result.text);
					if (cleaned.length > 0) {
						require_logger.logger_default.debug("[ChatKitProvider] Extracted response", {
					...
					require_logger.logger_default.debug("[ChatKitProvider] No assistant content found after cleaning", {
						originalLength: result.text.length,
						source: result.source
					});
				}
			} catch (e) {
				require_logger.logger_default.debug("[ChatKitProvider] Could not access frame", {
					url,
					error: e,
					attempt
				});
...
async function getIframeContent(page) {
	const frames = page.frames();
	require_logger.logger_default.debug("[ChatKitProvider] Checking frames", {
...
async function waitForContentStabilization(page, timeout, startTime) {
	...
	require_logger.logger_default.debug("[ChatKitProvider] Starting content stabilization polling");
	while (Date.now() - pollStartTime < timeout) {
		...
		if (pollElapsed % 5e3 < CONTENT_POLL_MS) require_logger.logger_default.debug("[ChatKitProvider] Polling state", {
		...
		if (currentContent !== lastContent) {
			require_logger.logger_default.debug("[ChatKitProvider] Content changed", {
		...
		if (!assistantMatch && currentContent.length > 0) require_logger.logger_default.debug("[ChatKitProvider] Assistant pattern not found, using full content for length check", { contentLength: currentContent.length });
		...
		if (!state.responding && timeSinceLastChange >= effectiveStabilizationMs && timeSinceStart >= effectiveMinWaitMs) {
			require_logger.logger_default.debug("[ChatKitProvider] Content stabilized", {
				timeSinceStart,
				timeSinceLastChange,
	...
	require_logger.logger_default.debug("[ChatKitProvider] Content stabilization timeout reached");
	return {
		assistantResponse: capturedAssistantResponse,
...
async function handleApproval(page, action) {
	...
	for (const frame of frames) if (isOpenAICdnUrl(frame.url())) try {
		...
		for (const selector of buttonSelectors) {
			...
			if (button) {
				if (await button.isVisible()) {
					require_logger.logger_default.debug("[ChatKitProvider] Found approval button, clicking", {
						action,
						selector
					});
					await button.click();
		...
		if (await frame.evaluate((btnText) => {
			...
			require_logger.logger_default.debug("[ChatKitProvider] Clicked approval button via evaluate", { action });
			await page.waitForTimeout(APPROVAL_CLICK_DELAY_MS);
			return true;
		}
	} catch (e) {
		require_logger.logger_default.debug("[ChatKitProvider] Error checking for approval buttons", { error: e });
	}
	return false;
}
/**
...
async function processApprovals(page, approvalHandling, maxApprovals, timeout) {
	...
	while (approvalCount < maxApprovals) {
		...
		approvalCount++;
		require_logger.logger_default.debug("[ChatKitProvider] Processed approval", {
			count: approvalCount,
			max: maxApprovals
		});
		try {
...
var OpenAiChatKitProvider = class OpenAiChatKitProvider extends require_openai.OpenAiGenericProvider {
	...
	*/
	async initialize() {
		...
		require_logger.logger_default.debug("[ChatKitProvider] Initializing", {
			workflowId,
		...
		await new Promise((resolve, reject) => {
			...
			this.server.listen(this.chatKitConfig.serverPort, () => {
				const address = this.server.address();
				this.serverPort = typeof address === "object" ? address?.port || 0 : 0;
				require_logger.logger_default.debug("[ChatKitProvider] Server started", { port: this.serverPort });
				resolve();
			});
		});
		try {
...
Original error: ${errorMessage}`);
		...
		this.page.on("console", (msg) => {
			const type = msg.type();
			if (type === "error" || type === "warning") require_logger.logger_default.debug("[ChatKitProvider] Browser console", {
				type,
				text: msg.text()
			});
		});
		await this.page.goto(`http://localhost:${this.serverPort}`, { waitUntil: "domcontentloaded" });
		require_logger.logger_default.debug("[ChatKitProvider] Waiting for ChatKit ready");
		...
		require_logger.logger_default.debug("[ChatKitProvider] Initialized successfully");
	}
	/**
	...
	*/
	async callApi(prompt, _context, _callApiOptions) {
		...
		require_logger.logger_default.debug("[ChatKitProvider] Starting call", {
		...
		try {
			...
			require_logger.logger_default.debug("[ChatKitProvider] Sending message", {
				stateful: this.chatKitConfig.stateful,
				isFollowUp,
				responseCount
			});
			...
			require_logger.logger_default.debug("[ChatKitProvider] Waiting for response");
			...
			if (approvalsHandled > 0) require_logger.logger_default.debug("[ChatKitProvider] Processed approvals", { count: approvalsHandled });
			let responseText = await extractResponseFromFrame(this.page);
			if (!responseText && stabilizationResult.assistantResponse) {
				require_logger.logger_default.debug("[ChatKitProvider] Using fallback content from stabilization", { fallbackLength: stabilizationResult.assistantResponse.length });
			...
			require_logger.logger_default.debug("[ChatKitProvider] Response received", {
				threadId,
			...
			require_logger.logger_default.error("[ChatKitProvider] Call failed", { error: errorMessage });
			if (this.page) try {
	...
	*/
	async callApiWithPool(prompt) {
		...
		try {
			...
			require_logger.logger_default.debug("[ChatKitProvider] Acquired page from pool", { stats: pool.getStats() });
			...
			if (approvalsHandled > 0) require_logger.logger_default.debug("[ChatKitProvider] Pool processed approvals", { count: approvalsHandled });
			let responseText = await extractResponseFromFrame(page);
			if (!responseText && stabilizationResult.assistantResponse) {
				require_logger.logger_default.debug("[ChatKitProvider] Pool using fallback content from stabilization", { fallbackLength: stabilizationResult.assistantResponse.length });
				responseText = cleanAssistantResponse(stabilizationResult.assistantResponse);
			}
			const threadId = await page.evaluate(() => window.__state.threadId);
			const latencyMs = Date.now() - startTime;
			require_logger.logger_default.debug("[ChatKitProvider] Pool response received", {
				threadId,
				textLength: responseText.length,
				latencyMs
			});
			...
			const errorMessage = error instanceof Error ? error.message : String(error);
			require_logger.logger_default.error("[ChatKitProvider] Pool call failed", { error: errorMessage });
...
//# sourceMappingURL=chatkit-Wzo9zOq7.cjs.map