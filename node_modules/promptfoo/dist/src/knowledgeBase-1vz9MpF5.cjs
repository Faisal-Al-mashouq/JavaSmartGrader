...
var AwsBedrockKnowledgeBaseProvider = class extends require_base.AwsBedrockGenericProvider {
	...
	async callApi(prompt) {
		...
		require_logger.logger_default.debug("Calling Amazon Bedrock Knowledge Base API", { params });
		...
		if (require_cache.isCacheEnabled()) {
			...
			if (cachedResponse) {
				require_logger.logger_default.debug(`Returning cached response for ${prompt}`);
		...
		try {
			...
			require_logger.logger_default.debug("Amazon Bedrock Knowledge Base API response", { response });
			let output = "";
			...
			if (require_cache.isCacheEnabled()) try {
				...
				require_logger.logger_default.error(`Failed to cache knowledge base response: ${String(err)}`);
			}
			return {
				output,
				metadata: { citations },
...
//# sourceMappingURL=knowledgeBase-1vz9MpF5.cjs.map