...
require('./telemetry-C7zkNdSS.cjs');
...
var NovaSonicProvider = class extends require_base.AwsBedrockGenericProvider {
	...
	async getBedrockClient() {
		...
		try {
			...
			require_logger.logger_default.error(`Error loading AWS SDK packages: ${categorized.message}`, { error: err });
	...
	async sendEvent(sessionId, event) {
		if (Object.keys(event.event)[0] !== "audioInput") require_logger.logger_default.debug("sendEvent: " + Object.keys(event.event)[0]);
		const session = this.sessions.get(sessionId);
		if (!session?.isActive) {
			require_logger.logger_default.error(`Session ${sessionId} is not active`);
			return;
		}
	...
	async endSession(sessionId) {
		...
		if (!session) throw new Error(`Session ${sessionId} not found`);
			...
			require_logger.logger_default.debug(`Session ${sessionId} is not active`);
			return;
		}
		...
		require_logger.logger_default.debug("Session closed");
	}
	async sendTextMessage(sessionId, role, prompt) {
		const session = this.sessions.get(sessionId);
		if (!session) throw new Error(`Session ${sessionId} not found`);
		const textPromptID = crypto.randomUUID();
		require_logger.logger_default.debug("sendTextMessage: " + prompt);
	...
	async callApi(prompt, context) {
		...
		require_logger.logger_default.debug("prompt: " + prompt.slice(0, 1e3));
		session.responseHandlers.set("textOutput", (data) => {
			require_logger.logger_default.debug("textOutput: " + JSON.stringify(data));
		...
		try {
			...
			require_logger.logger_default.debug("Sending sessionStart");
			...
			require_logger.logger_default.debug("Sending promptStart");
			...
			require_logger.logger_default.debug("Sending system prompt");
			await this.sendSystemPrompt(sessionId, context?.test?.metadata?.systemPrompt || "");
			require_logger.logger_default.debug("Processing conversation history");
			...
			try {
				...
				require_logger.logger_default.error(`Error processing conversation history: ${err}`);
			}
			require_logger.logger_default.debug("Sending audioInput start");
			await this.sendEvent(sessionId, { event: { contentStart: {
				promptName: session.promptName,
			...
			require_logger.logger_default.debug("Sending audioInput chunks");
			const chunks = promptText?.match(/.{1,1024}/g)?.map((chunk) => node_buffer.Buffer.from(chunk)) || [];
			require_logger.logger_default.debug("audioInput in chunks: " + chunks.length);
			...
			require_logger.logger_default.debug("Sending audioInput end");
			await this.sendEvent(sessionId, { event: { contentEnd: {
				promptName: session.promptName,
				contentName: session.audioContentId
			} } });
			...
			if (response.body) for await (const event of response.body) {
				...
				if (event.chunk?.bytes) {
					...
					require_logger.logger_default.debug("processing eventType: " + eventType);
			...
			require_logger.logger_default.error(`Nova Sonic provider error [${categorized.type}]: ${categorized.message}`, { error });
			return {
...
//# sourceMappingURL=nova-sonic-0ut5nkiM.cjs.map