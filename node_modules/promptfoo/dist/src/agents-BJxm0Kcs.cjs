...
async function loadAgentDefinition(agentConfig) {
	if (isAgentInstance(agentConfig)) {
		require_logger.logger_default.debug("[AgentsLoader] Using provided Agent instance");
		return agentConfig;
	}
	if (typeof agentConfig === "string" && agentConfig.startsWith("file://")) {
		require_logger.logger_default.debug("[AgentsLoader] Loading agent from file", { path: agentConfig });
		return await loadAgentFromFile(agentConfig);
	}
	if (typeof agentConfig === "object") {
		require_logger.logger_default.debug("[AgentsLoader] Creating agent from inline definition");
		return await createAgentFromDefinition(agentConfig);
	}
	require_logger.logger_default.debug("[AgentsLoader] Invalid agent configuration", {
...
async function loadAgentFromFile(filePath) {
	...
	require_logger.logger_default.debug("[AgentsLoader] Loading agent from resolved path", { path: resolvedPath });
	try {
		...
		require_logger.logger_default.error("[AgentsLoader] Failed to load agent from file", {
			path: resolvedPath,
			error
		});
...
async function createAgentFromDefinition(definition) {
	try {
		...
		require_logger.logger_default.error("[AgentsLoader] Failed to create agent from definition", {
			name: definition?.name,
			model: definition?.model,
...
function resolveFilePath(filePath) {
	...
	require_logger.logger_default.debug("[AgentsLoader] Resolved file path", {
		original: filePath,
		basePath,
...
var OTLPTracingExporter = class {
	...
	*/
	async export(items, signal) {
		if (items.length === 0) {
			require_logger.logger_default.debug("[AgentsTracing] No items to export");
			return;
		}
		require_logger.logger_default.debug(`[AgentsTracing] Exporting ${items.length} items to OTLP`);
		try {
			const otlpPayload = this.transformToOTLP(items);
			const url = `${this.otlpEndpoint}/v1/traces`;
			require_logger.logger_default.debug("[AgentsTracing] Sending OTLP payload", {
				url,
			...
			if (response.ok) require_logger.logger_default.debug("[AgentsTracing] Successfully exported traces to OTLP");
			else require_logger.logger_default.error(`[AgentsTracing] OTLP export failed: ${response.status} ${response.statusText}`);
		} catch (error) {
			require_logger.logger_default.error("[AgentsTracing] Failed to export traces to OTLP", { error });
		}
	}
	/**
	...
	*/
	hexToBase64(hex, kind) {
		...
		try {
			...
			require_logger.logger_default.error(`[AgentsTracing] Failed to convert hex to base64: ${hex}`, { error });
...
var OpenAiAgentsProvider = class extends require_openai.OpenAiGenericProvider {
	...
	*/
	async callApi(prompt, context, callApiOptions) {
		require_logger.logger_default.debug("[AgentsProvider] Starting agent call", {
		...
		try {
			...
			require_logger.logger_default.debug("[AgentsProvider] Agent run completed", {
				outputLength: result.output?.length || 0,
				tokenUsage: result.tokenUsage
			});
			return result;
		} catch (error) {
			require_logger.logger_default.error("[AgentsProvider] Agent call failed", { error });
			throw error;
		}
	}
	/**
	...
	*/
	async initializeAgent() {
		require_logger.logger_default.debug("[AgentsProvider] Initializing agent");
		...
		try {
			...
			require_logger.logger_default.debug("[AgentsProvider] Agent initialized successfully", { name: agent.name });
			return agent;
		} catch (error) {
			require_logger.logger_default.error("[AgentsProvider] Failed to initialize agent", { error });
	...
	*/
	async setupTracingIfNeeded(context) {
		if (!(this.agentConfig.tracing === true || context?.test?.metadata?.tracingEnabled === true || process.env.PROMPTFOO_TRACING_ENABLED === "true")) {
			require_logger.logger_default.debug("[AgentsProvider] Tracing not enabled");
			return;
		}
		require_logger.logger_default.debug("[AgentsProvider] Setting up tracing");
		try {
			...
			require_logger.logger_default.debug("[AgentsProvider] Tracing setup complete");
		} catch (error) {
			require_logger.logger_default.error("[AgentsProvider] Failed to setup tracing", { error });
		}
	}
	/**
	...
	*/
	async registerTracingExporter(exporter) {
		try {
			...
			require_logger.logger_default.debug("[AgentsProvider] Tracing processor registered");
		} catch (error) {
			require_logger.logger_default.error("[AgentsProvider] Failed to register tracing processor", { error });
			throw error;
		}
	}
	/**
	...
	*/
	async runAgent(prompt, context, callApiOptions) {
		try {
			require_logger.logger_default.debug("[AgentsProvider] Running agent", {
			...
			require_logger.logger_default.debug("[AgentsProvider] Agent run result", {
			...
			require_logger.logger_default.error("[AgentsProvider] Failed to run agent", { error });
			throw error;
		}
	}
	/**
...
//# sourceMappingURL=agents-BJxm0Kcs.cjs.map