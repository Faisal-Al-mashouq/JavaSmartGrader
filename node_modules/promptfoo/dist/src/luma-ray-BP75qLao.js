...
import { r as storeBlob } from "./blobs-zfeA0Dke.js";
import "./tables-Dy5j8PcW.js";
import "./telemetry-CBTAdhX5.js";
...
var LumaRayVideoProvider = class extends AwsBedrockGenericProvider {
	...
	*/
	async startVideoGeneration(modelInput, s3OutputUri) {
		try {
			...
			const error = err;
			logger_default.error("[Luma Ray] Failed to start video generation", { error });
	...
	async pollForCompletion(invocationArn, pollIntervalMs, maxPollTimeMs) {
		...
		try {
			...
			while (Date.now() - startTime < maxPollTimeMs) {
				...
				logger_default.debug(`[Luma Ray] Job status: ${invocation.status}`, {
					invocationArn,
			...
			const error = err;
			logger_default.error("[Luma Ray] Polling error", {
				error,
				invocationArn
			});
	...
	*/
	async downloadAndStoreVideo(s3Uri) {
		try {
			...
			logger_default.debug("[Luma Ray] Downloading video from S3", {
				bucket,
				key: videoKey
			});
			...
			logger_default.debug("[Luma Ray] Stored video to blob storage", {
				uri: ref.uri,
				hash: ref.hash
			});
			return { blobRef: ref };
			...
			const error = err;
			logger_default.error("[Luma Ray] S3 download error", {
				error,
				s3Uri
			});
	...
	*/
	...
	async callApi(prompt, context) {
		...
		logger_default.info("[Luma Ray] Starting video generation job...", {
		...
		logger_default.info("[Luma Ray] Job started", { invocationArn });
		...
		if (config.downloadFromS3 !== false) {
			...
			if (downloadError) logger_default.warn(`[Luma Ray] Failed to download video: ${downloadError}. Using S3 URL.`);
			else blobRef = ref;
		}
...
//# sourceMappingURL=luma-ray-BP75qLao.js.map