...
var SageMakerCompletionProvider = class extends SageMakerGenericProvider {
	...
	*/
	async callApi(prompt, context, _options) {
		const { isCacheEnabled, getCache } = await Promise.resolve().then(() => require("./cache-BZYapLG-.cjs"));
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (getCache ? getCache() : await Promise.resolve().then(() => require("./cache-BZYapLG-.cjs")).then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
		...
		try {
			...
			if (isCacheEnabled() && !bustCache && result.output && !result.error) {
				const cacheKey = this.getCacheKey(transformedPrompt);
				const cache = getCache ? getCache() : await Promise.resolve().then(() => require("./cache-BZYapLG-.cjs")).then((m) => m.getCache());
	...
	*/
	async callEmbeddingApi(text, context) {
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (await getCache ? await getCache() : await Promise.resolve().then(() => require("./cache-BZYapLG-.cjs")).then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
	...
	*/
	async cacheEmbeddingResult(result, text, context, isTransformed = false, originalText) {
		...
		if (isCacheEnabled() && !bustCache && result.embedding && !result.error) {
			...
			const cache = await getCache ? await getCache() : await Promise.resolve().then(() => require("./cache-BZYapLG-.cjs")).then((m) => m.getCache());
...
//# sourceMappingURL=sagemaker-BsttQmAf.cjs.map