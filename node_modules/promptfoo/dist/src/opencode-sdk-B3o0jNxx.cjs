...
require('./cache-DYJRe0Jt.cjs');
const require_cometapi = require('./cometapi-RP8h5O2f.cjs');
let fs = require("fs");
...
async function loadOpenCodeSDK() {
	try {
		require_logger.logger_default.debug("Attempting simple dynamic import of @opencode-ai/sdk");
		return await import("@opencode-ai/sdk");
	} catch {
		require_logger.logger_default.debug("Simple import failed, falling back to smart ESM resolution");
	}
	try {
		const modulePath = resolveEsmPackage("@opencode-ai/sdk", require_logger.cliState_default.basePath && path.default.isAbsolute(require_logger.cliState_default.basePath) ? require_logger.cliState_default.basePath : process.cwd());
		require_logger.logger_default.debug(`Resolved OpenCode SDK path: ${modulePath}`);
		return await require_esm.importModule(modulePath);
	} catch (err) {
		require_logger.logger_default.error(`Failed to load OpenCode SDK: ${err}`);
		const stack = err.stack;
		if (stack) require_logger.logger_default.error(stack);
	...
	*/
	...
	async cleanup() {
		if (!this.config.persist_sessions) for (const sessionId of this.sessions.values()) try {
			...
			require_logger.logger_default.debug(`Failed to delete session ${sessionId}: ${err}`);
		}
		this.sessions.clear();
		this.sessionOrder = [];
		if (this.server) {
			try {
				...
				require_logger.logger_default.debug(`Failed to close OpenCode server: ${err}`);
			}
			this.server = void 0;
		}
	}
	...
	*/
		...
		while (this.sessions.size >= MAX_SESSIONS && this.sessionOrder.length > 0) {
			...
			if (oldestKey) {
				...
				if (oldSessionId) this.client?.session?.delete({ path: { id: oldSessionId } }).catch((err) => {
					require_logger.logger_default.debug(`Failed to delete evicted session ${oldSessionId}: ${err}`);
				});
			}
		}
	...
	async callApi(prompt, context, callOptions) {
		...
		if (config.working_dir) {
			workingDir = path.default.isAbsolute(config.working_dir) ? config.working_dir : path.default.resolve(process.cwd(), config.working_dir);
			let stats;
			try {
		...
		try {
			...
			if (!this.client) {
				...
				if (config.baseUrl) this.client = createOpencodeClient({ baseUrl: config.baseUrl });
					...
					if (!serverEnv.PATH?.includes(opencodeBinPath)) {
						...
						require_logger.logger_default.debug(`Added ${opencodeBinPath} to PATH for OpenCode CLI`);
					}
					if (config.log_level === "debug" || isDebugMode()) {
						serverEnv.DEBUG = serverEnv.DEBUG || "opencode:*";
						require_logger.logger_default.debug("[OpenCode SDK] Debug mode enabled, synced from promptfoo log level");
					}
					const serverOptions = {
					...
					if (config.mcp && Object.keys(config.mcp).length > 0) {
						...
						require_logger.logger_default.debug(`[OpenCode SDK] Configuring MCP servers: ${Object.keys(config.mcp).join(", ")}`);
					}
					if (config.custom_agent) {
						...
						require_logger.logger_default.debug(`[OpenCode SDK] Configuring custom agent: ${config.custom_agent.description}`);
					}
					if (config.permission) {
						serverConfig.permission = config.permission;
						require_logger.logger_default.debug("[OpenCode SDK] Configuring global permissions");
					}
					...
					require_logger.logger_default.debug(`OpenCode server started at ${opencode.server.url}`);
				}
			}
			let sessionId;
			...
			require_logger.logger_default.debug(`OpenCode SDK prompt options:`, {
				path: promptOptions.path,
				body: promptBody,
				query: promptOptions.query
			});
			const response = await this.client.session.prompt(promptOptions);
			require_logger.logger_default.debug(`OpenCode SDK response received`);
			...
			require_logger.logger_default.debug(`OpenCode SDK response: ${output.substring(0, 100)}...`);
			return providerResponse;
		} catch (error) {
			if (error?.name === "AbortError" || callOptions?.abortSignal?.aborted) {
				require_logger.logger_default.warn("OpenCode SDK call aborted");
          ...
          Or see: https://opencode.ai for other installation methods.`;
				require_logger.logger_default.error(cliError);
				return { error: cliError };
			}
			require_logger.logger_default.error(`Error calling OpenCode SDK: ${error}`);
...
//# sourceMappingURL=opencode-sdk-B3o0jNxx.cjs.map