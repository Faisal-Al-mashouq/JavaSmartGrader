...
var ProviderRegistry = class {
	...
	registerShutdownHandlers() {
		...
		const shutdown = async (signal) => {
			...
			shuttingDown = true;
			require_logger.logger_default.debug(`Received ${signal}, shutting down ${this.providers.size} Python providers...`);
			await Promise.all(Array.from(this.providers).map((p) => p.shutdown().catch((err) => {
				require_logger.logger_default.error(`Error shutting down provider: ${err}`);
			})));
			require_logger.logger_default.debug("Python provider shutdown complete");
		};
	...
	async shutdownAll() {
		...
		for (const result of results) if (result.status === "rejected") require_logger.logger_default.warn(`Error shutting down provider: ${result.reason}`);
...
//# sourceMappingURL=providerRegistry-DFXamjVO.cjs.map