...
import { t as getDirectory } from "./esm-D40XsWlR.js";
import "./types-CyCiwPFa.js";
import "./tables-Dy5j8PcW.js";
...
async function loadProtoDefinitions() {
	...
	logger_default.debug("[Protobuf] Loading OTLP proto definitions");
	const protoDir = getProtoDir();
	logger_default.debug(`[Protobuf] Proto directory: ${protoDir}`);
	try {
		...
		protoRoot = root;
		logger_default.debug("[Protobuf] Successfully loaded OTLP proto definitions");
		return protoRoot;
	} catch (error) {
		logger_default.error(`[Protobuf] Failed to load proto definitions: ${error}`);
		throw error;
	}
}
/**
...
async function decodeExportTraceServiceRequest(data) {
	...
	try {
		...
		logger_default.debug(`[Protobuf] Decoded ExportTraceServiceRequest with ${decoded.resourceSpans?.length || 0} resource spans`);
		return decoded;
	} catch (error) {
		logger_default.error(`[Protobuf] Failed to decode ExportTraceServiceRequest: ${error}`);
...
var OTLPReceiver = class {
	...
	constructor() {
		...
		logger_default.debug("[OtlpReceiver] Initializing OTLP receiver");
	...
	setupMiddleware() {
		...
		logger_default.debug("[OtlpReceiver] Middleware configured for JSON and protobuf");
	}
	setupRoutes() {
		this.app.post("/v1/traces", async (req, res) => {
			const contentType = req.headers["content-type"] || "unknown";
			const bodySize = req.body ? JSON.stringify(req.body).length : 0;
			logger_default.debug(`[OtlpReceiver] Received trace request: ${req.headers["content-type"]} with ${bodySize} bytes`);
			logger_default.debug("[OtlpReceiver] Starting to process traces");
			...
			try {
				let traces = [];
				if (isJson) {
					logger_default.debug("[OtlpReceiver] Parsing OTLP JSON request");
					logger_default.debug(`[OtlpReceiver] Request body: ${JSON.stringify(req.body).substring(0, 500)}...`);
					traces = this.parseOTLPJSONRequest(req.body);
				} else if (isProtobuf) {
					logger_default.debug("[OtlpReceiver] Parsing OTLP protobuf request");
					logger_default.debug(`[OtlpReceiver] Request body size: ${req.body?.length || 0} bytes`);
					traces = await this.parseOTLPProtobufRequest(req.body);
				}
				logger_default.debug(`[OtlpReceiver] Parsed ${traces.length} traces from request`);
				...
				logger_default.debug(`[OtlpReceiver] Grouped spans into ${spansByTrace.size} traces`);
				for (const [traceId, info] of traceInfoById) try {
					logger_default.debug(`[OtlpReceiver] Creating trace record for ${traceId}`);
					...
					logger_default.debug(`[OtlpReceiver] Trace ${traceId} may already exist: ${error}`);
				}
				for (const [traceId, spans] of spansByTrace) {
					logger_default.debug(`[OtlpReceiver] Storing ${spans.length} spans for trace ${traceId}`);
					await this.traceStore.addSpans(traceId, spans, { skipTraceCheck: true });
				}
				res.status(200).json({ partialSuccess: {} });
				logger_default.debug("[OtlpReceiver] Successfully processed traces");
			} catch (error) {
				logger_default.error(`[OtlpReceiver] Failed to process OTLP traces: ${error}`);
				logger_default.error(`[OtlpReceiver] Error stack: ${error instanceof Error ? error.stack : "No stack"}`);
		...
		this.app.get("/health", (_req, res) => {
			logger_default.debug("[OtlpReceiver] Health check requested");
		...
		this.app.use((error, _req, res, _next) => {
			logger_default.error(`[OtlpReceiver] Global error handler: ${error}`);
			logger_default.error(`[OtlpReceiver] Error stack: ${error.stack}`);
	...
	parseOTLPJSONRequest(body) {
		const traces = [];
		logger_default.debug(`[OtlpReceiver] Parsing request with ${body.resourceSpans?.length || 0} resource spans`);
		for (const resourceSpan of body.resourceSpans) {
			const resourceAttributes = this.parseAttributes(resourceSpan.resource?.attributes);
			logger_default.debug(`[OtlpReceiver] Parsed ${Object.keys(resourceAttributes).length} resource attributes`);
			for (const scopeSpan of resourceSpan.scopeSpans) for (const span of scopeSpan.spans) {
				...
				logger_default.debug(`[OtlpReceiver] Processing span: ${span.name} (${spanId}) in trace ${traceId}`);
	...
	async parseOTLPProtobufRequest(body) {
		...
		logger_default.debug(`[OtlpReceiver] Parsing protobuf request with ${decoded.resourceSpans?.length || 0} resource spans`);
		for (const resourceSpan of decoded.resourceSpans || []) {
			...
			logger_default.debug(`[OtlpReceiver] Parsed ${Object.keys(resourceAttributes).length} resource attributes from protobuf`);
			for (const scopeSpan of resourceSpan.scopeSpans || []) for (const span of scopeSpan.spans || []) {
				...
				logger_default.debug(`[OtlpReceiver] Processing protobuf span: ${span.name} (${spanId}) in trace ${traceId}`);
	...
	convertId(id, expectedHexLength) {
		logger_default.debug(`[OtlpReceiver] Converting ID: ${id} (length: ${id.length}, expected hex length: ${expectedHexLength})`);
		if (id.length === expectedHexLength && /^[0-9a-f]+$/i.test(id)) {
			logger_default.debug(`[OtlpReceiver] ID is already hex format`);
			return id.toLowerCase();
		}
		try {
			const buffer = Buffer.from(id, "base64");
			const hex = buffer.toString("hex");
			logger_default.debug(`[OtlpReceiver] Base64 decoded: ${id} -> ${hex} (${buffer.length} bytes)`);
			const utf8String = buffer.toString("utf8");
			if (utf8String.length === expectedHexLength && /^[0-9a-f]+$/i.test(utf8String)) {
				logger_default.debug(`[OtlpReceiver] Detected hex string encoded as UTF-8: ${utf8String}`);
				return utf8String.toLowerCase();
			}
			if (hex.length === expectedHexLength) return hex;
			logger_default.warn(`[OtlpReceiver] Unexpected ID format: ${id} -> ${hex} (expected ${expectedHexLength} hex chars)`);
			return id.toLowerCase();
		} catch (error) {
			logger_default.error(`[OtlpReceiver] Failed to convert ID: ${error}`);
			return id.toLowerCase();
		}
	}
	listen(port = 4318, host = "127.0.0.1") {
		this.port = port;
		logger_default.debug(`[OtlpReceiver] Starting receiver on ${host}:${port}`);
		return new Promise((resolve, reject) => {
			this.server = this.app.listen(port, host, () => {
				logger_default.info(`[OtlpReceiver] Listening on http://${host}:${port}`);
				logger_default.debug("[OtlpReceiver] Receiver fully initialized and ready to accept traces");
				resolve();
			});
			this.server.on("error", (error) => {
				logger_default.error(`[OtlpReceiver] Failed to start: ${error}`);
				reject(error);
			});
		});
	}
	stop() {
		logger_default.debug("[OtlpReceiver] Stopping receiver");
		return new Promise((resolve) => {
			if (this.server) this.server.close(() => {
				logger_default.info("[OtlpReceiver] Server stopped");
				this.server = void 0;
				resolve();
			});
			else {
				logger_default.debug("[OtlpReceiver] No server to stop");
				resolve();
			}
		});
	}
...
async function startOTLPReceiver(port, host) {
	logger_default.debug("[OtlpReceiver] Starting receiver through startOTLPReceiver function");
	await getOTLPReceiver().listen(port, host);
}
async function stopOTLPReceiver() {
	logger_default.debug("[OtlpReceiver] Stopping receiver through stopOTLPReceiver function");
	if (otlpReceiver) {
...
//# sourceMappingURL=otlpReceiver-9PkCcnK9.js.map