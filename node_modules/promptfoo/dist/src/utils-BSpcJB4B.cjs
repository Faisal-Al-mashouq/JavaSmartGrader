...
function generateIdFromPrompt(prompt) {
	if (prompt.label) return require_extractor.sha256(prompt.label);
	if (prompt.id) return require_extractor.sha256(prompt.id);
	return require_extractor.sha256(typeof prompt.raw === "object" ? JSON.stringify(prompt.raw) : prompt.raw);
}
...
//# sourceMappingURL=utils-BSpcJB4B.cjs.map