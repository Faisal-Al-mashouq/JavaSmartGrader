...
*/
const MIGRATION_SUNSET_DATE = /* @__PURE__ */ new Date("2026-02-01T00:00:00Z");
/**
...
function getDirSize(dirPath) {
	...
	try {
		...
		for (const entry of entries) {
			const itemPath = path.default.join(dirPath, entry.name);
			if (entry.isDirectory()) totalSize += getDirSize(itemPath);
			else totalSize += fs.default.statSync(itemPath).size;
		}
	} catch (err) {
		if (err.code !== "ENOENT") require_logger.logger_default.warn(`[Cache Migration] Error calculating directory size: ${err.message}`);
	}
	return totalSize;
}
/**
...
function checkDiskSpace(cachePath) {
	try {
		...
		if (typeof fs.default.statfsSync === "function") {
			const stats = fs.default.statfsSync(cachePath);
			const availableBytes = stats.bavail * stats.bsize;
			const requiredBytes = cacheSize * 2 + 10 * 1024 * 1024;
			require_logger.logger_default.debug(`[Cache Migration] Disk space check: need ${(requiredBytes / 1024 / 1024).toFixed(2)}MB, have ${(availableBytes / 1024 / 1024).toFixed(2)}MB available`);
			if (availableBytes < requiredBytes) {
				require_logger.logger_default.error(`[Cache Migration] Insufficient disk space for migration. Need ${(requiredBytes / 1024 / 1024).toFixed(0)}MB, have ${(availableBytes / 1024 / 1024).toFixed(0)}MB available.`);
				return false;
			}
			return true;
		} else {
			require_logger.logger_default.debug("[Cache Migration] Disk space check not available on this platform, proceeding");
			return true;
		}
	} catch (err) {
		require_logger.logger_default.warn(`[Cache Migration] Could not check disk space: ${err.message}`);
		return true;
	}
}
...
function acquireMigrationLock(cachePath, attempt = 1) {
	if (attempt > MAX_LOCK_ATTEMPTS) {
		require_logger.logger_default.warn(`[Cache Migration] Failed to acquire lock after ${MAX_LOCK_ATTEMPTS} attempts`);
		return null;
	}
	const lockFile = path.default.join(cachePath, ".migration.lock");
	try {
		if (!fs.default.existsSync(cachePath)) fs.default.mkdirSync(cachePath, { recursive: true });
		const fd = fs.default.openSync(lockFile, "wx");
		fs.default.writeSync(fd, process.pid.toString());
		fs.default.fsyncSync(fd);
		require_logger.logger_default.debug(`[Cache Migration] Lock acquired (PID: ${process.pid})`);
		return fd;
	} catch (err) {
		if (err.code === "EEXIST") {
			try {
				const content = fs.default.readFileSync(lockFile, "utf-8");
				const pid = parseInt(content, 10);
				if (!isNaN(pid)) {
					if (isProcessRunning(pid)) {
						require_logger.logger_default.info(`[Cache Migration] Another migration is in progress (PID: ${pid})`);
						return null;
					}
					require_logger.logger_default.warn(`[Cache Migration] Removing stale lock file (PID: ${pid} not found)`);
					try {
						fs.default.unlinkSync(lockFile);
						return acquireMigrationLock(cachePath, attempt + 1);
					} catch (unlinkErr) {
						require_logger.logger_default.error(`[Cache Migration] Failed to remove stale lock: ${unlinkErr.message}`);
						return null;
					}
				}
			} catch (readErr) {
				require_logger.logger_default.warn(`[Cache Migration] Could not read lock file: ${readErr.message}`);
			}
			return null;
		}
		throw err;
...
function releaseMigrationLock(fd, cachePath) {
	...
	try {
		...
		require_logger.logger_default.warn(`[Cache Migration] Failed to close lock file: ${err.message}`);
	}
	try {
		fs.default.unlinkSync(lockFile);
		require_logger.logger_default.debug("[Cache Migration] Lock released");
	} catch (err) {
		require_logger.logger_default.warn(`[Cache Migration] Failed to remove lock file: ${err.message}`);
	}
}
/**
...
function readOldCacheEntries(cachePath) {
	...
	if (!fs.default.existsSync(cachePath)) {
		require_logger.logger_default.info(`[Cache Migration] No old cache directory found at ${cachePath}`);
		return {
			entries,
			stats
		};
	...
	require_logger.logger_default.info(`[Cache Migration] Found ${diskstoreDirs.length} diskstore directories`);
	const shouldLogProgress = diskstoreDirs.length > 100;
	if (shouldLogProgress) require_logger.logger_default.info(`[Cache Migration] Processing large cache, this may take a moment...`);
	const now = Date.now();
	let dirCount = 0;
	for (const dir of diskstoreDirs) {
		const dirPath = path.default.join(cachePath, dir);
		dirCount++;
		if (shouldLogProgress && dirCount % 100 === 0) require_logger.logger_default.info(`[Cache Migration] Processed ${dirCount}/${diskstoreDirs.length} directories...`);
		try {
			...
			for (const file of jsonFiles) {
				...
				stats.totalFiles++;
				if (stats.totalFiles % 1e3 === 0 && stats.totalFiles > 0) require_logger.logger_default.info(`[Cache Migration] Processed ${stats.totalFiles} files...`);
				try {
...
function validateCacheFile(cachePath, expectedEntryCount) {
	if (!fs.default.existsSync(cachePath)) throw new Error(`Cache file does not exist after write: ${cachePath}`);
	const content = fs.default.readFileSync(cachePath, "utf-8");
	let parsed;
	try {
	...
	require_logger.logger_default.debug(`[Cache Migration] Validated cache file: ${cachePath} (${expectedEntryCount} entries)`);
}
/**
...
function writeNewCacheFile(entries, newCachePath) {
	...
	const dir = path.default.dirname(newCachePath);
	if (!fs.default.existsSync(dir)) fs.default.mkdirSync(dir, { recursive: true });
	const tempFile = path.default.join(dir, `.cache.${(0, crypto.randomBytes)(8).toString("hex")}.tmp`);
	try {
		let serialized;
		try {
		...
		require_logger.logger_default.debug(`[Cache Migration] Atomically wrote cache file: ${newCachePath}`);
		validateCacheFile(newCachePath, entries.size);
	} catch (err) {
		try {
			if (fs.default.existsSync(tempFile)) fs.default.unlinkSync(tempFile);
		} catch (_cleanupErr) {}
		throw err;
	}
}
...
function createBackup(cachePath) {
	...
	require_logger.logger_default.info(`[Cache Migration] Creating backup at ${backupPath}`);
...
function markMigrationComplete(cacheBasePath, stats) {
	const markerPath = path.default.join(cacheBasePath, ".cache-migrated");
	const metadata = {
		timestamp: (/* @__PURE__ */ new Date()).toISOString(),
		stats,
		version: "4-to-7"
	};
	if (!fs.default.existsSync(cacheBasePath)) fs.default.mkdirSync(cacheBasePath, { recursive: true });
	fs.default.writeFileSync(markerPath, JSON.stringify(metadata, null, 2), "utf-8");
}
/**
...
function isMigrationComplete(cacheBasePath, newCacheFile) {
	const markerPath = path.default.join(cacheBasePath, ".cache-migrated");
	if (!fs.default.existsSync(markerPath)) return false;
	if (newCacheFile && fs.default.existsSync(newCacheFile)) return true;
	if (newCacheFile) {
		if (hasOldCacheFormat(cacheBasePath)) {
			require_logger.logger_default.warn("[Cache Migration] Marker file exists but migration appears incomplete. Old cache format found but new cache missing. Retrying migration...");
			try {
				fs.default.unlinkSync(markerPath);
			} catch (err) {
				require_logger.logger_default.warn(`[Cache Migration] Failed to remove stale marker: ${err.message}`);
			}
			return false;
		}
	}
...
function hasOldCacheFormat(cachePath) {
	let dirEntries;
	try {
		dirEntries = fs.default.readdirSync(cachePath, { withFileTypes: true });
	} catch (err) {
...
function cleanupOldCache(cachePath) {
	...
	require_logger.logger_default.info(`[Cache Migration] Cleaning up ${diskstoreDirs.length} old cache directories`);
	for (const dir of diskstoreDirs) {
		const dirPath = path.default.join(cachePath, dir);
		try {
			fs.default.rmSync(dirPath, {
				recursive: true,
				force: true
			});
		} catch (err) {
			require_logger.logger_default.warn(`[Cache Migration] Failed to remove ${dirPath}: ${err.message}`);
		}
	}
}
/**
...
function cleanupBackup(backupPath, stats) {
	if (stats.successCount === 0 && stats.failureCount === 0) {
		require_logger.logger_default.info(`[Cache Migration] No valid entries found (${stats.skippedExpired} expired only). Removing backup to save space.`);
		try {
			fs.default.rmSync(backupPath, {
				recursive: true,
				force: true
			});
			require_logger.logger_default.info(`[Cache Migration] Backup removed: ${backupPath}`);
			return true;
		} catch (err) {
			require_logger.logger_default.warn(`[Cache Migration] Failed to remove backup ${backupPath}: ${err.message}`);
			return false;
		}
	} else if (stats.failureCount > 0) {
		require_logger.logger_default.info(`[Cache Migration] Backup kept at ${backupPath} due to ${stats.failureCount} migration errors. You may want to investigate these failures.`);
		return false;
	} else {
		require_logger.logger_default.info(`[Cache Migration] Backup kept at ${backupPath} (migrated ${stats.successCount} valid entries). You can manually delete this backup if you no longer need it.`);
		return false;
	}
}
/**
...
function runMigration(cachePath, newCacheFilePath) {
	require_logger.logger_default.info("[Cache Migration] Starting cache migration from v4 to v7");
	const lock = acquireMigrationLock(cachePath);
	if (lock === null) {
		require_logger.logger_default.info("[Cache Migration] Another migration is in progress, skipping");
		return {
			success: true,
			stats: {
				totalFiles: 0,
	...
	try {
		if (isMigrationComplete(cachePath, newCacheFilePath)) {
			require_logger.logger_default.info("[Cache Migration] Migration already completed, skipping");
			return {
				success: true,
				stats: {
					totalFiles: 0,
		...
		if (!hasOldCacheFormat(cachePath)) {
			require_logger.logger_default.info("[Cache Migration] No old cache format detected, skipping migration");
		...
		if (!checkDiskSpace(cachePath)) {
			require_logger.logger_default.error("[Cache Migration] Insufficient disk space, aborting migration");
			return {
				success: false,
				stats: {
					totalFiles: 0,
		...
		require_logger.logger_default.info("[Cache Migration] Reading old cache entries");
		const { entries, stats } = readOldCacheEntries(cachePath);
		require_logger.logger_default.info(`[Cache Migration] Read ${stats.successCount} entries (${stats.failureCount} failures, ${stats.skippedExpired} expired)`);
		if (stats.errors.length > 0) {
			require_logger.logger_default.warn(`[Cache Migration] Encountered ${stats.errors.length} errors:`);
			stats.errors.slice(0, 10).forEach((err) => require_logger.logger_default.warn(`  - ${err}`));
			if (stats.errors.length > 10) require_logger.logger_default.warn(`  ... and ${stats.errors.length - 10} more errors`);
		}
		if (entries.size > 0) {
			require_logger.logger_default.info(`[Cache Migration] Writing ${entries.size} entries to new cache file: ${newCacheFilePath}`);
			writeNewCacheFile(entries, newCacheFilePath);
		} else require_logger.logger_default.info("[Cache Migration] No entries to migrate");
		cleanupOldCache(cachePath);
		const backupDeleted = cleanupBackup(backupPath, stats);
		markMigrationComplete(cachePath, stats);
		require_logger.logger_default.info("[Cache Migration] Migration completed successfully");
		return {
			success: true,
			stats,
		...
		require_logger.logger_default.error(`[Cache Migration] Migration failed: ${err.message}`);
		require_logger.logger_default.error(`[Cache Migration] Stack trace: ${err.stack}`);
		return {
			success: false,
			stats: {
				totalFiles: 0,
...
function getCache() {
	if (!cacheInstance) {
		...
		if (cacheType === "disk" && enabled) {
			...
			if (!fs.default.existsSync(cachePath)) {
				require_logger.logger_default.info(`Creating cache folder at ${cachePath}.`);
			...
			if (shouldRunMigration(cachePath, newCacheFile)) {
				require_logger.logger_default.info("[Cache] Migrating cache from v4 to v7...");
				try {
					const result = runMigration(cachePath, newCacheFile);
					if (result.success) {
						require_logger.logger_default.info(`[Cache] Migration completed: ${result.stats.successCount} entries migrated, ${result.stats.skippedExpired} expired`);
						if (result.backupPath) require_logger.logger_default.info(`[Cache] Backup kept at: ${result.backupPath}`);
					} else {
						require_logger.logger_default.error(`[Cache] Migration failed: ${result.stats.errors.join(", ")}. Falling back to memory cache.`);
						migrationFailed = true;
					}
				} catch (err) {
					require_logger.logger_default.error(`[Cache] Migration error: ${err.message}. Falling back to memory cache.`);
					migrationFailed = true;
				}
			}
			if (!migrationFailed) try {
				...
				require_logger.logger_default.warn(`[Cache] Failed to initialize disk cache: ${err.message}. Using memory cache instead.`);
			}
		}
...
async function fetchWithCache(url, options = {}, timeout = require_fetch.REQUEST_TIMEOUT_MS, format = "json", bust = false, maxRetries) {
	...
	const cachedResponse = await cache.wrap(cacheKey, async () => {
		cached = false;
		const fetchStart = Date.now();
		const response = await require_fetch.fetchWithRetries(url, options, timeout, maxRetries);
		...
		try {
			...
			if (format === "json" && parsedData?.error) {
				require_logger.logger_default.debug(`Not caching ${url} because it contains an 'error' key: ${parsedData.error}`);
				return data;
			}
			require_logger.logger_default.debug(`Storing ${url} response in cache with latencyMs=${fetchLatencyMs}: ${data}`);
			return data;
		} catch (err) {
			throw new Error(`Error parsing response from ${url}: ${err.message}. Received text: ${responseText}`);
		}
	});
	if (cached && cachedResponse) require_logger.logger_default.debug(`Returning cached response for ${url}: ${cachedResponse}`);
	...
	return {
		...
		deleteFromCache: async () => {
			...
			require_logger.logger_default.debug(`Evicted from cache: ${cacheKey}`);
		}
	};
}
...
//# sourceMappingURL=cache-DH-n6i0J.cjs.map