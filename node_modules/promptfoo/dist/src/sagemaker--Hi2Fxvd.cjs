	...
	*/
	async getCredentials() {
		if (this.config.accessKeyId && this.config.secretAccessKey) {
			require_logger.logger_default.debug("Using explicit credentials from config");
			return {
		...
		if (this.config.profile) {
			require_logger.logger_default.debug(`Using AWS profile: ${this.config.profile}`);
			try {
		...
		require_logger.logger_default.debug("Using default AWS credentials from environment");
	}
	/**
	...
	*/
	async getSageMakerRuntimeInstance() {
		if (!this.sagemakerRuntime) try {
			...
			require_logger.logger_default.debug(`SageMaker client initialized for region ${this.getRegion()}`);
		} catch {
	...
	*/
	async applyTransformation(prompt, context) {
		...
		try {
			...
			require_logger.logger_default.debug(`Applying transform to prompt for SageMaker endpoint ${this.getEndpointName()}`);
			if (typeof transformFn === "string" && !transformFn.startsWith("file://")) try {
				if (transformFn.includes("=>")) {
					...
					if (result === void 0 || result === null) {
						require_logger.logger_default.debug("Transform function returned null or undefined, using original prompt");
						return prompt;
					}
				...
				require_logger.logger_default.error(`Error executing inline transform: ${transformError}`);
			}
			else try {
				const { TransformInputType } = await Promise.resolve().then(() => require("./transform-XcotjzZX.cjs"));
				...
				require_logger.logger_default.error(`Error using transform utility: ${transformError}`);
			}
			require_logger.logger_default.warn(`Transform did not produce a valid result, using original prompt`);
			return prompt;
		} catch (_) {
			require_logger.logger_default.error(`Error applying transform to prompt: ${_}`);
			return prompt;
		}
	}
	/**
	...
	async extractFromPath(responseJson, pathExpression) {
		...
		try {
			if (pathExpression.startsWith("file://")) try {
				...
				require_logger.logger_default.warn(`Failed to transform response using file: ${error}`);
				return responseJson;
			}
			try {
				const result = new Function("json", `try { return ${pathExpression}; } catch(e) { return undefined; }`)(responseJson);
				if (result === void 0) {
					require_logger.logger_default.warn(`Path expression "${pathExpression}" did not match any data in the response`);
					require_logger.logger_default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
					return responseJson;
				}
				return result;
			} catch (error) {
				require_logger.logger_default.warn(`Failed to evaluate expression "${pathExpression}": ${error}`);
				return responseJson;
			}
		} catch (error) {
			require_logger.logger_default.warn(`Failed to extract data using path expression "${pathExpression}": ${error}`);
			require_logger.logger_default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
			return responseJson;
		}
	}
...
var SageMakerCompletionProvider = class extends SageMakerGenericProvider {
	...
	*/
	formatPayload(prompt) {
		...
		let payload;
		require_logger.logger_default.debug(`Formatting payload for model type: ${this.modelType}`);
	...
	*/
	async parseResponse(responseBody) {
		let responseJson;
		require_logger.logger_default.debug(`Parsing response for model type: ${this.modelType}`);
		try {
			responseJson = JSON.parse(responseBody);
		} catch {
			require_logger.logger_default.debug("Response is not JSON, returning as-is");
			return responseBody;
		}
		if (this.config.responseFormat?.path) try {
			...
			require_logger.logger_default.warn(`Failed to extract from path: ${this.config.responseFormat.path}, Error: ${error}`);
			require_logger.logger_default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
			return responseJson;
		}
		if (responseJson.generated_text) {
			require_logger.logger_default.debug("Detected JumpStart model response format with generated_text field");
	...
	*/
	async callApi(prompt, context, _options) {
		const { isCacheEnabled, getCache } = await Promise.resolve().then(() => require("./cache-CBUAeE_t.cjs"));
		...
		if (isTransformed) {
			require_logger.logger_default.debug(`Prompt transformed for SageMaker endpoint ${this.getEndpointName()}`);
			require_logger.logger_default.debug(`Original: ${prompt.substring(0, 100)}${prompt.length > 100 ? "..." : ""}`);
			require_logger.logger_default.debug(`Transformed: ${transformedPrompt.substring(0, 100)}${transformedPrompt.length > 100 ? "..." : ""}`);
		}
		...
		if (isCacheEnabled() && !bustCache) {
			...
			const cachedResult = await (getCache ? getCache() : await Promise.resolve().then(() => require("./cache-CBUAeE_t.cjs")).then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
				require_logger.logger_default.debug(`Using cached SageMaker response for ${this.getEndpointName()}`);
				try {
					...
					require_logger.logger_default.warn(`Failed to parse cached SageMaker response: ${_}`);
				}
			}
		}
		if (delayMs && delayMs > 0) {
			require_logger.logger_default.debug(`Applying delay of ${delayMs}ms before calling SageMaker endpoint ${this.getEndpointName()}`);
			await sleep(delayMs);
		}
		const runtime = await this.getSageMakerRuntimeInstance();
		const payload = this.formatPayload(transformedPrompt);
		require_logger.logger_default.debug(`Calling SageMaker endpoint ${this.getEndpointName()}`);
		require_logger.logger_default.debug(`With payload: ${payload.length > 1e3 ? payload.substring(0, 1e3) + "..." : payload}`);
		try {
			...
			if (!response.Body) {
				require_logger.logger_default.error("No response body returned from SageMaker endpoint");
				return { error: "No response body returned from SageMaker endpoint" };
			}
			const responseBody = new TextDecoder().decode(response.Body);
			require_logger.logger_default.debug(`SageMaker response (truncated): ${responseBody.length > 1e3 ? responseBody.substring(0, 1e3) + "..." : responseBody}`);
			...
			if (typeof output === "object" && output !== null && "code" in output) {
				...
				if (Number.isInteger(code) && code === 424) {
					...
					require_logger.logger_default.error(errorMessage);
			...
			if (isCacheEnabled() && !bustCache && result.output && !result.error) {
				const cacheKey = this.getCacheKey(transformedPrompt);
				const cache = getCache ? getCache() : await Promise.resolve().then(() => require("./cache-CBUAeE_t.cjs")).then((m) => m.getCache());
				const resultToCache = JSON.stringify(result);
				try {
					await cache.set(cacheKey, resultToCache);
					require_logger.logger_default.debug(`Stored SageMaker response in cache with key: ${cacheKey.substring(0, 100)}...`);
				} catch (_) {
					require_logger.logger_default.warn(`Failed to store SageMaker response in cache: ${_}`);
				}
			}
			return result;
		} catch (error) {
			require_logger.logger_default.error(`SageMaker API error: ${error}`);
...
var SageMakerEmbeddingProvider = class extends SageMakerGenericProvider {
	...
	*/
	async callEmbeddingApi(text, context) {
		...
		if (isTransformed) {
			require_logger.logger_default.debug(`Text transformed for SageMaker embedding endpoint ${this.getEndpointName()}`);
			require_logger.logger_default.debug(`Original: ${text.substring(0, 100)}${text.length > 100 ? "..." : ""}`);
			require_logger.logger_default.debug(`Transformed: ${transformedText.substring(0, 100)}${transformedText.length > 100 ? "..." : ""}`);
		}
		const bustCache = context?.debug === true;
		if (isCacheEnabled() && !bustCache) {
			const cacheKey = this.getCacheKey(transformedText);
			const cachedResult = await (await getCache ? await getCache() : await Promise.resolve().then(() => require("./cache-CBUAeE_t.cjs")).then((m) => m.getCache())).get(cacheKey);
			if (cachedResult) {
				require_logger.logger_default.debug(`Using cached SageMaker embedding response for ${this.getEndpointName()}`);
				try {
					const parsedResult = JSON.parse(cachedResult);
					...
					require_logger.logger_default.warn(`Failed to parse cached SageMaker embedding response: ${_}`);
				}
			}
		}
		if (delayMs && delayMs > 0) {
			require_logger.logger_default.debug(`Applying delay of ${delayMs}ms before calling SageMaker embedding endpoint ${this.getEndpointName()}`);
			await sleep(delayMs);
		}
		const runtime = await this.getSageMakerRuntimeInstance();
		let payload;
		const modelType = this.config.modelType || "custom";
		require_logger.logger_default.debug(`Formatting embedding payload for model type: ${modelType}`);
		switch (modelType) {
			case "openai":
		...
		require_logger.logger_default.debug(`Calling SageMaker embedding endpoint ${this.getEndpointName()}`);
		require_logger.logger_default.debug(`With payload: ${payload}`);
		try {
			const { InvokeEndpointCommand } = await import("@aws-sdk/client-sagemaker-runtime");
			const command = new InvokeEndpointCommand({
				EndpointName: this.getEndpointName(),
			...
			Date.now() - startTime;
			if (!response.Body) {
				require_logger.logger_default.error("No response body returned from SageMaker embedding endpoint");
				return { error: "No response body returned from SageMaker embedding endpoint" };
			}
			const responseBody = new TextDecoder().decode(response.Body);
			require_logger.logger_default.debug(`SageMaker embedding response: ${responseBody.substring(0, 200)}...`);
			let responseJson;
			try {
				responseJson = JSON.parse(responseBody);
			} catch (_) {
			...
			if (this.config.responseFormat?.path) try {
				...
				if (Array.isArray(extracted) && extracted.every((val) => typeof val === "number")) {
					...
					return result;
				} else require_logger.logger_default.warn("Extracted data is not a valid embedding array, trying other extraction methods");
			} catch (error) {
				require_logger.logger_default.warn(`Failed to extract embedding from path expression: ${this.config.responseFormat.path}, Error: ${error}`);
				require_logger.logger_default.debug(`Response JSON structure: ${JSON.stringify(responseJson).substring(0, 200)}...`);
			}
			...
			require_logger.logger_default.error(`SageMaker embedding API error: ${error}`);
	...
	*/
	async cacheEmbeddingResult(result, text, context, isTransformed = false, originalText) {
		...
		if (isCacheEnabled() && !bustCache && result.embedding && !result.error) {
			...
			const cache = await getCache ? await getCache() : await Promise.resolve().then(() => require("./cache-CBUAeE_t.cjs")).then((m) => m.getCache());
			...
			try {
				await cache.set(cacheKey, resultToCache);
				require_logger.logger_default.debug(`Stored SageMaker embedding response in cache with key: ${cacheKey.substring(0, 100)}...`);
			} catch (_) {
				require_logger.logger_default.warn(`Failed to store SageMaker embedding response in cache: ${_}`);
			}
		}
	}
};
...
//# sourceMappingURL=sagemaker--Hi2Fxvd.cjs.map