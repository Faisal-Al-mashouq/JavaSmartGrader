...
export default function InstructorCourseDetail() {
  ...
  if (!Number.isFinite(id)) return <p className="text-red-400 text-sm">Invalid course.</p>;
    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:items-start max-w-6xl">
        ...
        {loading ? (
          ...
          <>
            ...
            <header className="space-y-2">
              <p className="text-xs font-semibold text-indigo-400 uppercase tracking-widest">Course</p>
              <h1 className="text-3xl font-extrabold text-white tracking-tight">{course.name}</h1>
              {course.description ? (
                <p className="text-slate-400 text-sm leading-relaxed max-w-3xl">{course.description}</p>
              ) : (
            ...
            <section id="course-assignments"
              className="rounded-2xl border border-white/[0.08] bg-slate-900/70 backdrop-blur-sm shadow-xl overflow-hidden scroll-mt-24">
              <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-bold text-slate-600 bg-slate-800 px-2 py-0.5 rounded-full tabular-nums">
                  {assignments.length}
                </span>
              </div>
              ...
              {assignments.length === 0 ? (
                <div className="px-5 py-10 text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-700">Create one with <strong className="text-slate-500">+ New assignment</strong> on the right.</p>
                </div>
              ) : (
                  ...
                  {assignments.map((a, idx) => (
                    <li key={a.id}>
                      <Link to={`/instructor/courses/${id}/assignments/${a.id}`}
                        ...
                        <span className="w-6 h-6 rounded-lg bg-indigo-500/15 border border-indigo-500/20 flex items-center justify-center text-[10px] font-bold text-indigo-400 shrink-0">
                          {idx + 1}
                        </span>
                        <div className="min-w-0 flex-1">
                          ...
                          {a.description && (
                            <p className="text-xs text-slate-600 mt-0.5 line-clamp-1">{a.description}</p>
                          )}
                          {a.due_date && (
                        ...
                        <svg className="w-4 h-4 text-slate-700 group-hover:text-indigo-400 transition-colors shrink-0"
            ...
            {/* Students section */}
            <section className="rounded-2xl border border-white/[0.08] bg-slate-900/70 backdrop-blur-sm shadow-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
              ...
              {enrolled.length === 0 ? (
                <div className="px-5 py-10 text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-700">Enroll students from the panel on the right.</p>
                </div>
              ) : (
                <ul className="divide-y divide-white/[0.04]">
                  {enrolled.map((u) => (
                    <li key={u.id} className="px-5 py-3.5 flex items-center justify-between gap-3 hover:bg-white/[0.02] transition-colors group">
                      ...
                      <button type="button" disabled={busy} onClick={() => handleUnenroll(u.id)}
                        className="text-xs font-semibold text-slate-700 hover:text-red-400 disabled:opacity-50 transition-colors shrink-0">
                        Remove
                      </button>
                    </li>
                  ))}
      ...
      <aside className="lg:w-56 shrink-0 flex flex-col gap-3 lg:sticky lg:top-24">
        <Link to={`/instructor/courses/${id}/assignments/new`}
          className="block text-center text-sm font-semibold px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white shadow-lg shadow-indigo-900/30 transition-all">
          + New assignment
        </Link>
        ...
        <button type="button"
          ...
          className={`w-full text-sm font-semibold px-4 py-2.5 rounded-xl border transition-all duration-200
            ${showEnroll
              ...
              : "border-white/[0.08] text-slate-300 bg-slate-800/60 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
            }`}>
        ...
        {showEnroll && course && enrollOptions.length > 0 && (
          <form onSubmit={handleEnroll}
            className="rounded-2xl border border-white/[0.08] bg-slate-900/80 backdrop-blur-sm p-4 space-y-3 shadow-xl">
            <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1">
              Select student
            </label>
            <select
              className="w-full rounded-xl border border-white/[0.08] bg-slate-800/60 px-3 py-2 text-sm text-white focus:ring-2 focus:ring-indigo-500 outline-none"
            ...
            <button type="submit" disabled={busy}
              className="w-full text-sm font-semibold py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white disabled:opacity-50 shadow-lg transition-all">
