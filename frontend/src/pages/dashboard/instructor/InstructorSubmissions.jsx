...
function StatusBadge({ state }) {
  ...
  const cls   = STATUS_CLS[state]  ?? "bg-slate-100 text-slate-500 border border-slate-200";
...
export default function InstructorSubmissions() {
  const [all,          setAll]          = useState([]); // enriched submission rows
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Processing",  value: pending,    iconCls: "text-amber-600 dark:text-amber-400",   bg: "bg-amber-50 dark:bg-amber-900/30",   path: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
          ...
          { label: "Failed",      value: failed,     iconCls: "text-red-600 dark:text-red-400",       bg: "bg-red-50 dark:bg-red-900/30",       path: "M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" },
          ...
          <div key={label} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm px-5 py-4 flex items-center gap-3 hover:shadow-md transition-shadow">
            ...
            <div>
              ...
              <p className="text-lg font-extrabold text-slate-900 dark:text-white">{loading ? "…" : value}</p>
            </div>
          </div>
        ))}
      </div>
              ...
              {loading ? (
                ...
                <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right">
                    {s.state === "graded"
                      ? <a href="/instructor/grading" className="text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg transition-colors active:scale-95">Review AI</a>
