...
function ScoreRing({ score, max }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div
        className={`w-24 h-24 rounded-full border-4 ${ringColor} flex flex-col items-center justify-center`}
      >
        <span className={`text-2xl font-extrabold leading-none ${txtColor}`}>
          {Math.round(score)}
        </span>
...
function AIGradingPanel({ sub, onPublished }) {
  ...
  const doPublish = async () => {
    if (selectedGrade == null) {
      setOverrideErr("Select an accepted or override grade before publishing.");
      return;
    }
    setPublishing(true);
    try {
      try {
        await addGrade(sub.id, selectedGrade);
      } catch (e) {
        if (e.response?.status === 409) {
          await updateGrade(sub.id, selectedGrade);
        } else {
          throw e;
        }
      }
      await publishGrade(sub.id);
      setPublished(true);
      onPublished(sub.id, selectedGrade);
    } catch (e) {
      ...
      <div className="bg-white dark:bg-slate-900/70 dark:backdrop-blur-sm rounded-2xl border border-emerald-200 dark:border-emerald-500/25 shadow-sm dark:shadow-xl px-6 py-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="text-sm font-bold text-slate-900 dark:text-white">
            Grade Published — Submission #{sub.id}
          </p>
  ...
  return (
    <div className="bg-white dark:bg-slate-900/70 dark:backdrop-blur-sm rounded-2xl border border-indigo-200 dark:border-indigo-500/25 shadow-md dark:shadow-xl overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="space-y-2">
            ...
            {showCode && sub.transcription?.transcribed_text && (
              <pre className="bg-slate-900 dark:bg-black/50 text-emerald-400 text-xs rounded-xl p-4 overflow-x-auto leading-relaxed font-mono border border-slate-700 dark:border-white/[0.08]">
                {sub.transcription.transcribed_text}
              </pre>
            )}
            {showCode && !sub.transcription?.transcribed_text && (
              <p className="text-xs text-slate-400 dark:text-slate-500 italic px-1">
                No OCR transcription available.
              </p>
            )}
          </div>
        ...
        {!overrideMode ? (
          <div className="flex gap-3">
            ...
            >
              ...
              Accept AI Grade ({Math.round(suggestedGrade)}/100)
            </button>
            <button
          ...
          <div className="space-y-3 border border-slate-200 dark:border-white/[0.08] rounded-xl p-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl border border-slate-200 dark:border-white/[0.1] text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        {selectedGrade != null && (
          <div className="rounded-xl border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/10 px-4 py-3 flex items-center justify-between gap-3">
            <p className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">
              Ready to publish: {Math.round(selectedGrade)}/100
            </p>
            <button
              onClick={doPublish}
              disabled={publishing}
              className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:opacity-70 text-white text-xs font-bold transition-colors active:scale-95"
            >
        ...
        const enriched = await Promise.all(
          gradedSubs.map(async (s) => {
            ...
            return {
              ...
              transcription: tRes?.data ?? null,
              grade: gRes?.data ?? null,
            };
          }),
        );

        if (!cancelled) {
          const unpublished = enriched.filter((s) => !s.grade?.published_at);
          setAiGraded(unpublished);
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Review AI-graded submissions and publish grades
          </p>
        </div>
        {totalDone > 0 && (
      ...
      {!loading && totalAll > 0 && (
        <div className="bg-white dark:bg-slate-900/70 dark:backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-white/[0.08] shadow-sm dark:shadow-xl px-6 py-4">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1.5">
            {pendingReview.length} remaining to review
          </p>
        </div>
      )}
      ...
      {!loading && processing.length > 0 && (
        <div className="space-y-3">
          ...
          {processing.map((sub) => (
            <div
              key={sub.id}
              className="bg-white dark:bg-slate-900/70 dark:backdrop-blur-sm rounded-2xl border border-blue-200 dark:border-blue-500/25 shadow-sm dark:shadow-xl px-6 py-5 flex items-center gap-4"
            >
              <svg
                className="w-6 h-6 text-blue-500 animate-spin shrink-0"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500">
                  Student #{sub.student_id} · Submission #{sub.id} ·{" "}
                  {new Date(sub.submitted_at).toLocaleDateString()}
                </p>
              </div>
              <span className="ml-auto text-xs font-semibold bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full border border-blue-200 dark:border-blue-500/25">
                Processing…
              </span>
            </div>
          ))}
