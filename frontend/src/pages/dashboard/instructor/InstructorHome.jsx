...
/* ── helpers ─────────────────────────────────────────────────────── */
const STATUS_CLS = {
  submitted:  "bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800",
  processing: "bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800",
  graded:     "bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800",
...
function StatusBadge({ state }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_CLS[state] ?? "bg-slate-100 text-slate-500 border border-slate-200"}`}>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard
          title="Courses" value={courses.length} subtitle="Active courses"
        ...
        />
        <StatCard
          title="Processing" value={loading ? "…" : pending} subtitle="Being graded by AI"
        ...
        />
        <StatCard
          title="AI Graded" value={loading ? "…" : aiGraded} subtitle="Ready to review"
        ...
        />
        <StatCard
          title="Total Submissions" value={loading ? "…" : allSubs.length} subtitle="Across all courses"
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {loading ? (
                <tr><td colSpan={6} className="text-center py-12 text-slate-400 dark:text-slate-500 text-sm">Loading…</td></tr>
              ) : recentSubs.length === 0 ? (
                ...
                <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors group">
                  <td className="px-6 py-4">
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 font-mono">Student #{s.student_id}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">Sub #{s.id}</p>
                  </td>
                  <td className="px-4 py-4">
                    <p className="text-sm text-slate-700 dark:text-slate-300 font-medium max-w-[180px] truncate">{s.assignment?.title ?? `#${s.assignment_id}`}</p>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-1 rounded-md">{s.course?.name ?? `Course #${s.assignment?.course_id}`}</span>
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link
                      to="/instructor/grading"
                      className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 px-3 py-1.5 rounded-lg transition-colors"
                    >
            ...
            return (
              <div key={c.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {courseSubs.filter((s) => s.state === "submitted" || s.state === "processing").length > 0 && (
                    <span className="text-xs font-semibold bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-full">
                      {courseSubs.filter((s) => s.state === "submitted" || s.state === "processing").length} processing
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-400 dark:text-slate-500">Total Submissions</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{courseSubs.length}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-400 dark:text-slate-500">AI Graded</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{courseSubs.filter((s) => s.state === "graded").length}</span>
                  </div>
                </div>
              </div>
            );
