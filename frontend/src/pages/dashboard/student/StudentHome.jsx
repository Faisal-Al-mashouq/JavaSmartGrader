...
function StatusBadge({ status }) {
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[status] ?? "bg-slate-400"} ${status === "Processing" ? "animate-pulse" : ""}`}
      />
      {status}
    </span>
  );
...
const selectCls =
  "text-sm text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm";
...
export default function StudentHome() {
        ...
        const built = subs.map((s) => {
          ...
          return {
            ...
            dueDate: asgn?.due_date
              ? new Date(asgn.due_date).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })
  ...
  const avgGrade =
    graded.length > 0
      ? (
          graded.reduce(
            (s, r) => s + Math.round((r.grade / r.maxGrade) * 100),
            0,
          ) / graded.length
        ).toFixed(1)
      : "—";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <StatCard
          ...
          value={rows.length}
          subtitle="Submitted this semester"
            ...
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700">
                {[
                  ...
                  <th
                    key={h}
                    className={`text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider py-3 ${i === 0 ? "text-left px-6" : "text-left px-4"}`}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {loading ? (
                ...
                <tr>
                  ...
                  >
                    {rows.length === 0
                      ? "No submissions yet. Submit your first exam!"
                ...
                filtered.map((r) => (
                  <tr
                    key={r.id}
                    className="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors duration-100"
                  >
                    ...
                    <td className="px-4 py-4">
                      {r.grade !== null ? (
                        ...
                        <span className="text-slate-300 dark:text-slate-600 text-sm font-medium">
                          —
                        </span>
                      )}
                    </td>
